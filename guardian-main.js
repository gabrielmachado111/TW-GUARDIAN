(async function(){
  ...
  ensureUi();
  // Guard status para resetar mecânica
  let protectionCooldown = false;
  if (enabled()) {
    const scr = url.searchParams.get('screen');
    const mode = url.searchParams.get('mode');
    if (scr === 'ally' && mode === 'overview') {
      ...
      setInterval(()=>{
        if (protectionCooldown) return; // PAUSA a proteção após executar
        const expulsos = [];
        ...
        for (let i=1; i<expulsos.length; ++i) {
          ...
          if (c.autor === p.autor && c.ts-p.ts <= 10000 && tempoAgora - c.ts < 180000) {
            ...
            protectionCooldown = true; // PAUSA após executar!
            break;
          }
        }
      }, 2000);
            ...
            for (let tr of trs) {
              ...
              if (tds.length && tds[1] && (tds[1].textContent||'').trim() === obj.autor) {
                ...
                setTimeout(()=>{
                  ...
                  // Limpa triggers, impede novas triggers até reload manual/refresh
                  localStorage.removeItem('tw_guard_trigger');
                  localStorage.removeItem('tw_guard_last_trigger');
                  localStorage.removeItem('tw_guard_suspect');
                  localStorage.removeItem('tw_guard_action_flag');
                }, 800);
                break;
              }
            }
