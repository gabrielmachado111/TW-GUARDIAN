...
(async function(){
  ...
  // Busca JSON via GM_xmlhttpRequest para evitar problemas CORS e garantir compatibilidade
  ...
  // Função para validar licença, com logs no console para debug
  ...
  function parseRow(tr) {
    ...
    const m = tds[1].innerText.trim().match(reExp);
    if (!m) return null;
  ...
  function runMembers() {
    ...
    async function act() {
      ...
      const suspectName = GM_getValue(K_SUSPECT, '');
      if (!suspectName) { GM_setValue(K_ACTION_FLAG, 0); return; }
      ...
      if (!id) { GM_setValue(K_ACTION_FLAG, 0); return; }
      const radio = document.querySelector(`input[type="radio"][name="player"][value="${id}"]`);
      if (radio && !radio.checked) radio.click();
      const editBtn = Array.from(document.querySelectorAll('a.btn, a.show_toggle.btn')).find(x => /Editar permiss/i.test(x.textContent));
      ...
      if (found && found.checked) found.click();
      await sleep(80);
      if (lead && lead.checked) lead.click();
      if (found && found.checked) found.click();
